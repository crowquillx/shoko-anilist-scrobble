...
deps="$publish_dir/Shoko.AniListScrobble.deps.json"
dll="$publish_dir/Shoko.AniListScrobble.dll"
...
jq -e --arg expected "$expected" '
  ([((.targets // {})[] | keys[] | select(startswith("Shoko.AniListScrobble/")) | split("/")[1])] | unique) == [$expected]
  and ([((.libraries // {}) | keys[] | select(startswith("Shoko.AniListScrobble/")) | split("/")[1])] | unique) == [$expected]
' "$deps" >/dev/null \
