...
#   Shoko.AniListScrobble-<version>-any.zip       the portable plugin archive
#   Shoko.AniListScrobble-<version>-any.zip.sha256  SHA-256 sidecar (sha256sum -c compatible)
...
if [[ -z "$repository_url" ]]; then
  ...
  case "$remote" in
    ...
    *)
      repository_url="$(sed -n 's/.*<RepositoryUrl[^>]*>\(.*\)<\/RepositoryUrl>.*/\1/p' "$root/src/Shoko.AniListScrobble.csproj" | head -n1)"
      ;;
  esac
...
dotnet publish "$root/src/Shoko.AniListScrobble.csproj" -c "$configuration" --no-self-contained -o "$publish_dir" "${msbuild_args[@]}"
...
zip_name="Shoko.AniListScrobble-${version}-any.zip"
