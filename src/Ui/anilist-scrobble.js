(() => {
  ...
  const PLUGIN_ID = 'a3e8c1f0-4b7d-4e9a-9c2f-6d1b8a5e3f70';
  const PLUGIN_API = `${API_ROOT}/Plugin/AniListScrobble`;
  const SECRET_FIELDS = ['ClientId'];
  const CONFIG_FIELDS = [
    ['Enabled', 'boolean'],
  ...
  const populateConfiguration = (config, schema) => {
    ...
    for (const name of SECRET_FIELDS) {
      ...
      if (help) help.textContent = state.secrets[name] === '' ? 'No stored client ID. Enter one after creating an AniList API client.' : 'Stored value is present. Leave blank to keep it.';
    }
  ...
  const renderStatus = status => {
    ...
    const connected = propertyValue(status, 'Connected') === true;
    const version = propertyValue(status, 'PluginVersion');
    const username = propertyValue(status, 'AnilistUsername');
    const versionText = typeof version === 'string' ? ` Plugin version ${version}.` : '';
    const accountText = connected ? (typeof username === 'string' && username !== '' ? ` Connected as ${username}.` : ' Connected.') : ' Not connected.';
    setText('status-summary', `${enabled ? 'Scrobble is enabled.' : 'Scrobble is disabled.'}${accountText}${versionText}`);
    ...
    if (list) {
      ...
      if (!Array.isArray(capabilities) || capabilities.length === 0) {
        ...
        for (const capability of capabilities) {
          ...
          item.textContent = `${typeof name === 'string' ? name : 'Capability'}: ${capabilityEnabled ? 'enabled' : 'disabled'}`;
          list.append(item);
        }
      }
    }

    const details = getElement('status-details');
    if (!details) return;
    details.replaceChildren();
    appendDetail(details, 'Shoko user', String(propertyValue(status, 'ShokoUsername') ?? propertyValue(status, 'ShokoUserId') ?? 'unset'));
    appendDetail(details, 'Episodes remembered', String(propertyValue(status, 'ScrobbledCount') ?? 0));
    appendDetail(details, 'Scrobbles sent', String(propertyValue(status, 'Scrobbled') ?? 0));
    appendDetail(details, 'Completed', String(propertyValue(status, 'Completed') ?? 0));
    appendDetail(details, 'Last error', String(propertyValue(status, 'LastError') ?? 'none'));
    const last = parseObject(propertyValue(status, 'LastScrobble'));
    if (last) {
      appendDetail(details, 'Last AniList ID', String(propertyValue(last, 'AnilistId') ?? ''));
      appendDetail(details, 'Last progress', String(propertyValue(last, 'Progress') ?? ''));
      appendDetail(details, 'Last status', String(propertyValue(last, 'Status') ?? ''));
    }
  };
  ...
  const loadConfiguration = async () => {
    ...
    if (configId === '') throw new Error('The AniList Scrobble configuration was not found.');
  ...
  const loadStatus = async () => renderStatus(await requestJson(`${PLUGIN_API}/status`));

  const openAuthorize = async () => {
    const button = getElement('authorize-button');
    if (button) button.disabled = true;
    try {
      const result = await requestJson(`${PLUGIN_API}/auth/authorize-url`);
      const url = propertyValue(result, 'Url');
      if (typeof url !== 'string' || url === '') throw new Error('AniList authorize URL was empty.');
      globalThis.open(url, '_blank', 'noopener,noreferrer');
      setText('auth-instructions', 'Authorize the app, copy the token from the pin page, paste it here, then save.');
      showMessage('AniList authorize page opened.', 'success');
    } catch (error) {
      showMessage(error instanceof Error ? error.message : 'Unable to open AniList authorize.', 'error');
    } finally {
  ...
  const connectToken = async () => {
    ...
    const field = getElement('access-token');
    if (button) button.disabled = true;
    try {
      const token = field && 'value' in field ? String(field.value).trim() : '';
      if (token === '') throw new Error('Paste the AniList access token first.');
      const result = await requestJson(`${PLUGIN_API}/auth/token`, {
        method: 'POST',
        body: JSON.stringify({ apiVersion: 1, accessToken: token }),
      });
      if (field) field.value = '';
      const username = propertyValue(result, 'Username');
      setText('auth-instructions', typeof username === 'string' && username !== '' ? `Connected as ${username}.` : 'AniList account connected.');
      await loadStatus();
      showMessage('AniList account connected.', 'success');
    } catch (error) {
      showMessage(error instanceof Error ? error.message : 'Unable to save the AniList token.', 'error');
    } finally {
      if (button) button.disabled = false;
    }
  };
  ...
  const disconnect = async () => {
    const button = getElement('disconnect-button');
    if (button) button.disabled = true;
    try {
      renderStatus(await requestJson(`${PLUGIN_API}/auth/disconnect`, { method: 'POST' }));
      setText('auth-instructions', 'AniList account disconnected.');
      showMessage('AniList account disconnected.', 'success');
    } catch (error) {
      showMessage(error instanceof Error ? error.message : 'Unable to disconnect.', 'error');
    } finally {
      if (button) button.disabled = false;
    }
  };
  ...
  const refresh = async () => {
    ...
    setFormBusy(true);
    showMessage('Loading AniList Scrobble data.');
    try {
      const results = await Promise.allSettled([loadConfiguration(), loadStatus()]);
      const failure = results.find(result => result.status === 'rejected');
      if (failure && failure.status === 'rejected') {
        showMessage(failure.reason instanceof Error ? failure.reason.message : 'Unable to load AniList Scrobble data.', 'error');
      } else {
        showMessage('AniList Scrobble data is current.', 'success');
      }
    } finally {
      setFormBusy(false);
  ...
  const start = () => {
    const form = getElement('configuration-form');
    const refreshButton = getElement('refresh-button');
    const authorizeButton = getElement('authorize-button');
    ...
    if (authorizeButton) authorizeButton.addEventListener('click', openAuthorize);
    if (connectButton) connectButton.addEventListener('click', connectToken);
